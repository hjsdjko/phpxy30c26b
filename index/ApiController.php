...
class ApiController extends CommonController
{
    ...
    public function download()
    {
        ...
        header('location:http://' . $_SERVER['HTTP_HOST'] . '/phpxy30c26b/upload/' . $fileName);
        exit; 
    }
    /**
     * 获取某表的某个字段列表接口
    ...
    public function group($tablesName,$columnName){
        ...
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
            if($tokens['tablename'] == 'chahuazuopin'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'xiaoxitixing'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'jubaoxinxi'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `jubaoren` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'sixin'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'sixin'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `sixinzhanghao` = '".$tokens['username']."' ";
                }
            }
        }
    ...
    public function value($tableName,$xColumnName,$yColumnName,$tjType){
        ...
        $where = " where 1 ";
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
            if($tableName == 'chahuazuopin'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'xiaoxitixing'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'jubaoxinxi'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `jubaoren` = '".$tokens['username']."' ";
                }
            }

...
<div class="content" :style='{"minHeight":"100vh","padding":"40px 30px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20231104/93c9cca5595b4e21bb2a11e440cdca8d.png) no-repeat center top / cover,#f6f6f6","display":"flex","fontSize":"14px","height":"100%"}'>
	...
	<div :style='{"border":"0px solid rgba(126, 96, 16, .1)","padding":"0px 0","margin":"0 0px 20px 30px","alignItems":"center","color":"#7331f2","flexWrap":"wrap","background":"none","display":"flex","width":"200px","fontSize":"14px","justifyContent":"center","order":"2"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"8px 0","margin":"0 0 10px 0","borderColor":"#eee","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"space-around","borderRadius":"0px","background":"#fff","borderWidth":"1px","width":"100%","borderStyle":"solid"}' v-if="isAuth('yonghu','首页总数')">
		...
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"8px 0","margin":"0 0 10px 0","borderColor":"#eee","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"space-around","borderRadius":"0px","background":"#fff","borderWidth":"1px","width":"100%","borderStyle":"solid"}' v-if="isAuth('chahuazuopin','首页总数')">
			<div :style='{"alignItems":"center","borderRadius":"100%","background":"#fff","display":"flex","width":"48px","justifyContent":"center","height":"48px"}'>
				<span class="icon iconfont icon-tongji7" :style='{"color":"inherit","fontSize":"32px"}'></span>
			</div>
			<div :style='{"width":"auto","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{chahuazuopinCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>插画作品总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type3" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto","order":"10"}'>
		...
		<div id="chahuazuopinChart1" class="echarts2" v-if="isAuth('chahuazuopin','首页统计')"></div>
		<div id="chahuazuopinChart2" class="echarts3" v-if="isAuth('chahuazuopin','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getyonghuCount();
		this.yonghuChat1();
		this.getchahuazuopinCount();
		this.chahuazuopinChat1();
		this.chahuazuopinChat2();
	},
	methods:{
		// 词云
		...
		getchahuazuopinCount() {
			this.$http({
				url: `chahuazuopin/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.chahuazuopinCount = data.data
				}
			})
		},
		chahuazuopinChat1() {
        ...
        var chahuazuopinChart1 = echarts.init(document.getElementById("chahuazuopinChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                chahuazuopinChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    chahuazuopinChart1.resize();
                };
            }
        });
      })
    ...
    chahuazuopinChat2() {
      this.$nextTick(()=>{
        ...
        var chahuazuopinChart2 = echarts.init(document.getElementById("chahuazuopinChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                chahuazuopinChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    chahuazuopinChart2.resize();
                };
            }
        });
      })
